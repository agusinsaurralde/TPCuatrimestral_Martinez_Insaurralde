...
namespace WebApplication1
{
    public partial class Formulario_web113 : System.Web.UI.Page
    {
        ...
        protected void ddlistEspecialidad_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            List<MedicoEspecialidades> listaEspAgregadas = (List<MedicoEspecialidades>)Session["listaEspAgregadas"];
            MedicoEspecialidades espRepetida = new MedicoEspecialidades();
            lblDias.Visible = true;
            ddlistDias.Visible = true;

           /* espRepetida = listaEspAgregadas.Find(x => x.ID == int.Parse(ddlistEspecialidad.SelectedValue));
            if (espRepetida == null)
            {
                lblDias.Visible = true;
                ddlistDias.Visible = true;
        ...
        {
            List<DiasHabilesMedico> listaDiasAgregados = (List<DiasHabilesMedico>)Session["listaDiasAgregados"];
            DiasHabilesMedico diaRepetido = new DiasHabilesMedico();

           /* diaRepetido = listaDiasAgregados.Find(x => x.IdDia == ddlistDias.SelectedIndex);
            if (diaRepetido == null)
            {
        ...
        protected void Click_Aceptar(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<DiasHabilesMedico> listaDias = (List<DiasHabilesMedico>)Session["listaDiasAgregados"];
                List<MedicoEspecialidades> listaEsp = (List<MedicoEspecialidades>)Session["listaEspAgregadas"];
        ...
        {
            //espAgregada.Medico.ID
            List<DiasHabilesMedico> listaDiasAgregados = new List<DiasHabilesMedico>();
            ...
            listaDiasAgregados.Add(diaAgregado);
            Session.Add("listaDiasAgregados", listaDiasAgregados);
            ...
            for(int x = 0; x < 7; x++)
            {
                ...
                {
                    if (ddlistDias.Items[x].Text == obj.NombreDia)
                    {
        ...
        {
            List<MedicoEspecialidades> listaEspAgregadas = new List<MedicoEspecialidades>();
            ...
            listaEspAgregadas.Add(espAgregada);
           ...
           /* for (int x = 0; x <= y; x++)
            ...
            Session.Add("listaEspAgregadas", listaEspAgregadas);
